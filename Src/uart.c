...
#define APB1_PRESCALER 	1			// Prescaler is the factor by which clock is divided
...
void uart2_tx_init(void){
	...
	// Set PA3 alternate function type to UART2 RX (AF07)
	GPIOA->AFR[0] &= ~(1U<<15);
	GPIOA->AFR[0] |= (1U<<14);
	GPIOA->AFR[0] |= (1U<<13);
	GPIOA->AFR[0] |= (1U<<12);
...
void uart2_rxtx_init(void){
	...
	// Configure transfer direction
	// Control Register 1
	USART2->CR1 = (CR1_TE | CR1_RE);	// Not using OR, everything else becomes 0 in the register
	// Enable uart modules
	USART2->CR1 |= CR1_UE;
...
char uart2_read(void){
	// Check if the Read Data Register is not empty
